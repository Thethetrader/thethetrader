...
- [⚠️ Checkpoint Technique 10 Nov 2025](#️-checkpoint-technique-10-nov-2025)
...
## ⚠️ Checkpoint Technique 10 Nov 2025

**Commit de référence :** `a698031c6470dd1c68885b46041588c32621d3ce`  
**But :** disposer d’un point de retour stable après les ajustements UX/statistiques du 10 novembre 2025.

- **Landing page PWA**
  - Le CTA `Commencer Maintenant` scrolle désormais vers `#pricing` (hero principal + bloc `Journal Personnel`).
  - Nettoyage sécurisé des styles injectés pour supprimer les warnings TypeScript lors du démontage.
- **Statistiques signaux / calendrier**
  - `Avg Win` et `Avg Loss` utilisent uniquement la somme des trades gagnants/perdants, évitant les moyennes nulles sur un mois négatif.
  - `Weekly Breakdown` suit exactement les mêmes semaines LUN→DIM que l’affichage du calendrier pour aligner chiffres et UI.
- **Hook `useCalendarSync`**
  - Nouvelles accumulations `monthWinPnL` / `monthLossPnL` afin d’alimenter les métriques avec les bonnes valeurs discrètes.

💡 *Rollback rapide :* `git reset --hard a698031c6470dd1c68885b46041588c32621d3ce && npm run dev`

---

## ⚠️ Checkpoint Technique - Harmonisation Couleurs et Positionnement

**Commit de référence :** `a3e2676`  
**Date :** Aujourd'hui  
**But :** Point de retour stable après harmonisation des couleurs statistiques et amélioration du positionnement des éléments UI.

- **Harmonisation couleurs statistiques (admin + utilisateur desktop)**
  - Win Rate, Signaux actifs, Total Trades : couleur blanche (uniforme entre admin et utilisateur)
  - P&L Total : vert si positif, rouge si négatif (conditionnel des deux côtés)
- **Positionnement UI amélioré**
  - Légende calendrier (WIN/LOSS/BREAK/NO TRADE/Today) déplacée directement sous le calendrier dans journal perso PWA utilisateur
  - Graphique PnL cumulé déplacé sous le calendrier dans journal perso admin desktop (au lieu du panneau de stats à droite)
- **Fichiers modifiés**
  - `src/components/AdminInterface.tsx` : harmonisation couleurs + positionnement graphique PnL
  - `src/components/generated/TradingPlatformShell.tsx` : harmonisation couleurs + positionnement légende calendrier

💡 *Rollback rapide :* `git reset --hard a3e2676 && npm run dev`
