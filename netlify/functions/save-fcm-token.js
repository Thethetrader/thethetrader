...
export const handler = async (event) => {
  ...
  const { user_key, token, role } = body;
  if (!user_key || !token) return { statusCode: 400, headers: hdrs, body: JSON.stringify({ error: 'user_key and token required' }) };

  const supabase = createClient(SUPABASE_URL, serviceKey);

  const { error } = await supabase
    .from('push_tokens')
    .upsert({ user_key, token, role: role || 'user', updated_at: new Date().toISOString() }, { onConflict: 'user_key' });
