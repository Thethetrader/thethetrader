...
function SessionRoom({ isAdmin, otherName, onEnd, conversationId, senderType, senderName, senderEmail }: SessionRoomProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900">
      {/* Header */}
      <div className="flex items-center justify-between px-4 border-b border-gray-700 bg-gray-800 flex-shrink-0" style={{ height: 56 }}>
        {/* Left: status + name */}
        <div className="flex items-center gap-2 min-w-0">
          <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse flex-shrink-0" />
          <span className="text-sm text-white font-semibold truncate">
            {otherName || (isAdmin ? 'Utilisateur' : 'Admin')}
          </span>
        </div>
