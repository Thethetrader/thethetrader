        ...
        <div className="md:hidden bg-gray-800 border-b border-gray-600 overflow-x-auto">
          <div className="flex space-x-4 p-6 min-w-max">
            {channels.map((channel) => (
              <button key={channel.id} onClick={() => { if (channel.id === 'calendrier') { setView('calendar'); } else { setSelectedChannel({id: channel.id, name: channel.id}); setView('signals'); } }} className={`px-8 py-6 rounded-xl text-lg font-bold whitespace-nowrap min-w-[120px] ${ (channel.id === 'calendrier' && view === 'calendar') || (channel.id === selectedChannel.id && view === 'signals') ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-700 text-gray-300 hover:bg-gray-600' }`}>{channel.name}</button>
            ))}
          </div>
        </div>
